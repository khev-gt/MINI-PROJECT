...
void addTeam(int id, string name) {
    ...
    if (findTeamIndex(id) != -1) {
        cout << "Error: Team ID " << id << " already exists. Registration rejected.\n";
        return;
    }
...
void recordMission(int id, int points) {
    ...
    if (index == -1) {
        cout << "Error: No team found with ID " << id << ".\n";
        return;
    }

    if (points < 1 || points > 100) {
        cout << "Error: Points must be between 1 and 100. No changes made.\n";
        return;
    }
...
int main() {
    ...
    while (choice != 6) {
        ...
        // --- UX fix: handle non-numeric menu input safely ---
        // Without this, typing a letter here leaves cin stuck in a
        // "fail" state forever, and the menu prints "Invalid choice"
        // in an infinite loop without ever waiting for input again.
        if (cin.fail()) {
            cin.clear();            // reset the error flag on cin
            cin.ignore(1000, '\n'); // discard the bad input
            choice = 0;              // falls through to "Invalid choice" below
        }
